function s_hat=ML_Detection_QPSK_LimiedSet(r,H,W)

s_candidate_set=[]; % Define the candidate set
dist_set=[]; % Define the distance set

for b1_candidate=[1+j, -1-j, -1+j] % Candidate of antenna 1 includes {-1-j, -1+j, 1-j, 1+j} for QPSK modulation
    for b2_candidate=[1-j, -1+j] % Candidate of antenna 2 includes {-1-j, -1+j, 1-j, 1+j} for QPSK modulation
