...
for j = 1:length(P) 
    ...
    for i = 2:max_iteration 
        ...
        if abs(objective_function(i) - objective_function_current) <= stop_criteria
            if flag == 0 
                objective_function_Bob_convergent(j) = objective_function_Bob(i);
                objective_function_Eve_convergent(j) = objective_function_Eve(i);
                flag = 1;
            end 
        end
...
figure(1)
semilogy(SNR_dB,objective_function_Bob_convergent,'r');
grid on
hold on
semilogy(SNR_dB,objective_function_Eve_convergent,'g');
hold off
xlabel('SNR');
ylabel('Probability');
legend('Error Probability of Bob', 'Error Probability of Eve');