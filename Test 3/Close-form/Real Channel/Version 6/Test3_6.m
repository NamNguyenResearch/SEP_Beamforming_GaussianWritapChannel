...
for j = 1:number_test 
    ...
    if (flag == 1) 
        if (objective_function_Eve_convergent(j) < objective_function_Eve_constraint) && (objective_function_convergent(j) >= -0.8)
            j_min = j;
...
for j = 1:number_test
    if (j_convergent(j) ~= 0) 
        if (j ~= j_min) && (objective_function_convergent(j) >= -0.8) 
