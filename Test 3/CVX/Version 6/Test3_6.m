...
%==========================================================================
number_test = 50; 
objective_function_constraint = 100000000;
max_iteration = 600;
...
for j = 1:number_test 
    ...
    W = W_0;
    flag = 0;
     j_convergent(j) = max_iteration + 1000;
    ...
    if (flag == 1) 
        if (objective_function_convergent(j) < objective_function_constraint) && (objective_function_convergent(j) >= -1) 
            j_min = j;
            objective_function_constraint = objective_function_convergent(j);
        end
    end 
end
...
for j = 1:number_test 
    if (j_convergent(j) ~= 0) 
        if (j ~= j_min) && (objective_function_convergent(j) >= -1) 
            plot(iterations,objective_function(:,j),'-k');
            grid on;
            hold on;
        end
    end
...
if (j == number_test) 
    plot(iterations,objective_function(:,j),'-k');
    hold on
    plot(iterations,objective_function(:,j_min),'--r');
    grid on;
    xlabel('Iterations');
    ylabel('Objective value');
    legend('Other','Optimal');
    hold off;
end